...
var (
    ...
    visibilityTimeOut = kingpin.Flag("timeout", "SQS queue visibility timeout in seconds").Short('t').Default("2").Int64()
    waitTimeSeconds   = kingpin.Flag("wait", "The duration (in seconds) for which the call waits for a message to arrive in the queue").Short('w').Default("0").Int64()
